...
static int kv_list_helper(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
    ...
    memcpy((*bdev_io)->u.bdev.nvme_kv.key, key, key_length);
    (*bdev_io)->u.bdev.nvme_kv.key_length = key_length;
...
int spdk_bdev_kv_list(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
    ...
    int rc = kv_list_helper(desc, ch, key, key_length, cb, cb_arg, &bdev_io);
    if (rc) {
        return rc;
    }
...
int spdk_bdev_kv_listv(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
    ...
    bdev_io->u.bdev.nvme_kv.buffer_size = nbytes;
    bdev_io_submit(bdev_io);
    return 0;        
}
...
static int kv_store_helper(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
                   unsigned char *key, size_t key_length,
                   uint8_t options,
                   spdk_bdev_io_completion_cb cb, void *cb_arg, struct spdk_bdev_io **bdev_io) {
    if (key_length == 0 || key_length > NVME_KV_MAX_KEY_LENGTH) {
    ...
    memcpy((*bdev_io)->u.bdev.nvme_kv.key, key, key_length);
    (*bdev_io)->u.bdev.nvme_kv.key_length = key_length;
    (*bdev_io)->u.bdev.nvme_kv.options = options;
    bdev_io_init((*bdev_io), bdev, cb_arg, cb);
    return 0;
}
...
int spdk_bdev_kv_store(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
                   ...
                   spdk_bdev_io_completion_cb cb, void *cb_arg) {
    struct spdk_bdev_io *bdev_io;
    int rc = kv_store_helper(desc, ch, key, key_length, options, cb, cb_arg, &bdev_io);
    if (rc) {
        return rc;
    }
...
static int kv_retrieve_helper(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
                   unsigned char *key, size_t key_length,
                   uint64_t offset,
                   spdk_bdev_io_completion_cb cb, void *cb_arg, struct spdk_bdev_io **bdev_io) {
    if (key_length == 0 || key_length > NVME_KV_MAX_KEY_LENGTH) {
    ...
    memcpy((*bdev_io)->u.bdev.nvme_kv.key, key, key_length);
    (*bdev_io)->u.bdev.nvme_kv.key_length = key_length;
    (*bdev_io)->u.bdev.nvme_kv.offset = offset;
    bdev_io_init((*bdev_io), bdev, cb_arg, cb); 
    return 0;
}
...
int spdk_bdev_kv_retrieve(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
                   ...
                   spdk_bdev_io_completion_cb cb, void *cb_arg) {
    struct spdk_bdev_io *bdev_io;
    int rc = kv_retrieve_helper(desc, ch, key, key_length, offset, cb, cb_arg, &bdev_io);
    if (rc) {
        return rc;
    }
    bdev_io->u.bdev.iovs = &bdev_io->iov;
...
static int kv_send_select_helper(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
                   unsigned char *key, size_t key_length,
                   uint8_t options,
    ...
    (*bdev_io)->u.bdev.nvme_kv.key_length = key_length;
    (*bdev_io)->u.bdev.nvme_kv.options = options;
    (*bdev_io)->u.bdev.nvme_kv.select_input_type = input_type;
    (*bdev_io)->u.bdev.nvme_kv.select_output_type = output_type;
    bdev_io_init((*bdev_io), bdev, cb_arg, cb);
    return 0;
}
...
int spdk_bdev_kv_send_select(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
    ...
    struct spdk_bdev_io *bdev_io;
    int rc = kv_send_select_helper(desc, ch, key, key_length, options, input_type, output_type, cb, cb_arg, &bdev_io);
    if (rc) {
        return rc;
    }
...
static int kv_retrieve_select_helper(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
                   uint64_t offset,
    ...
    (*bdev_io)->u.bdev.nvme_kv.offset = offset;
    (*bdev_io)->u.bdev.nvme_kv.options = options;
    (*bdev_io)->u.bdev.nvme_kv.select_id = select_id;
    bdev_io_init((*bdev_io), bdev, cb_arg, cb);
    return 0;
}
...
int spdk_bdev_kv_retrieve_select(struct spdk_bdev_desc *desc, struct spdk_io_channel *ch,
                   ...
                   spdk_bdev_io_completion_cb cb, void *cb_arg) {
    struct spdk_bdev_io *bdev_io;
    int rc = kv_retrieve_select_helper(desc, ch, offset, select_id, options, cb, cb_arg, &bdev_io);
    if (rc) {
        return rc;
    }
